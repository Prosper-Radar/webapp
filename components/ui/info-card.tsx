...
const InfoCardDescription = React.memo(({ children, className, ...props }: InfoCardDescriptionProps) => (
  <div className={cn("text-muted-foreground leading-4", className)} {...props}>{children}</div>
));
InfoCardDescription.displayName = "InfoCardDescription";

interface CommonCardProps extends React.HTMLAttributes<HTMLDivElement> { children: React.ReactNode; }
...
type InfoCardDismissProps  = React.HTMLAttributes<HTMLDivElement> & { children: React.ReactNode; onDismiss?: () => void; };
...
const InfoCardContext = createContext<{ isHovered: boolean; onDismiss: () => void; }>({
  isHovered: false,
  onDismiss: () => {},
});
...
function InfoCard({ children, className, storageKey, dismissType = "once" }: InfoCardProps) {
  ...
  const imageContextValue = useMemo(() => ({ handleMediaLoad: () => {}, setAllImagesLoaded: () => {} }), []);
  const cardContextValue  = useMemo(() => ({ isHovered, onDismiss: handleDismiss }), [isHovered, handleDismiss]);
...
const InfoCardDismiss = React.memo(({ children, className, onDismiss, ...props }: InfoCardDismissProps) => {
  ...
  const handleClick = (e: React.MouseEvent) => { e.preventDefault(); onDismiss?.(); contextDismiss(); };
  return <div className={cn("cursor-pointer", className)} onClick={handleClick} {...props}>{children}</div>;
});
...
const InfoCardMedia = ({ media = [], className, loading, shrinkHeight = 75, expandHeight = 150 }: InfoCardMediaProps) => {
  const { isHovered }            = useContext(InfoCardContext);
  ...
  const processedMedia = useMemo(() => media.map((item) => ({ ...item, type: item.type || "image" })), [media]);
  ...
  useEffect(() => {
    ...
    if (isHovered) { id = setTimeout(() => setIsOverflowVisible(true), 100); }
    else setIsOverflowVisible(false);
    return () => clearTimeout(id);
  }, [isHovered]);

  const count = displayMedia.length;
  const getRotation  = (i: number) => (!isHovered || count === 1) ? 0 : (i - (count === 2 ? 0.5 : 1)) * 5;
  const getTranslateX = (i: number) => (!isHovered || count === 1) ? 0 : (i - (count === 2 ? 0.5 : 1)) * 20;
  const getTranslateY = (i: number) => !isHovered ? 0 : (count === 1 ? -5 : i === 0 ? -10 : i === 1 ? -5 : 0);
  const getScale      = (i: number) => !isHovered ? 1 : (count === 1 ? 1 : 0.95 + i * 0.02);
      ...
      >
        <div className="relative" style={{ height: shrinkHeight }}>
          {displayMedia.map((item, index) => {
            const { type, src, alt, className: cls, ...rest } = item;
            return (
              <motion.div
                key={src}
                className="absolute w-full"
                animate={{ rotateZ: getRotation(index), x: getTranslateX(index), y: getTranslateY(index), scale: getScale(index) }}
                transition={{ type: "spring", stiffness: 300, damping: 30 }}
              >
                {type === "video" ? (
                  <video src={src} className={cn("w-full rounded-md border border-border object-cover shadow-lg", cls)} onLoadedData={() => handleMediaLoad(src)} preload="metadata" muted playsInline {...(rest as React.VideoHTMLAttributes<HTMLVideoElement>)} />
                ) : (
                  <img src={src} alt={alt} className={cn("w-full rounded-md border border-border object-cover shadow-lg", cls)} onLoad={() => handleMediaLoad(src)} loading={loading} />
                )}
              </motion.div>
            );
          })}
